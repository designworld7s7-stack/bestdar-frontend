	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			...
			if (userId) {
				...
				// When email confirmation is enabled, user object might be null until confirm
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
			<div className="w-full max-w-md bg-white rounded-lg shadow-md p-6">
				<h1 className="text-2xl font-bold text-gray-900 mb-4 text-center">Create account</h1>
					...
					<div>
						...
						<div className="mt-3">
							...
							<ul className="mt-2 grid grid-cols-1 gap-1 text-xs">
								...
								<li className={/[0-9]/.test(password) ? 'text-green-600' : 'text-gray-500'}>• One number</li>
								<li className={/[^A-Za-z0-9]/.test(password) ? 'text-green-600' : 'text-gray-500'}>• One symbol (e.g. !@#$%)</li>
							</ul>
						</div>
					</div>
