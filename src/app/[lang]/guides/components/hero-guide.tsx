...
export default function HeroGuide({ id, title, description, image, date, lang }: any) {
  ...
  return (
    <section className="w-full max-w-[1440px] mx-auto px-4 lg:px-12 mb-12 lg:mb-24">
      {/* 1. DYNAMIC LINK: Correctly points to /[lang]/guides/[slug] */}
      <Link 
        href={`/${lang}/guides/${id}`} 
        className="group relative block w-full min-h-[500px] lg:aspect-[21/9] rounded-[32px] lg:rounded-[40px] overflow-hidden bg-gray-900 shadow-2xl"
      >
        
        {/* 2. DYNAMIC IMAGE: Using standard img for best compatibility with your Seed */}
        <img 
        ...
        {/* 3. OVERLAY: High-end gradient for text readability */}
        ...
        <div className="absolute inset-0 p-8 md:p-12 lg:p-20 flex flex-col justify-end items-start z-20">
          ...
          <h2 className="text-[32px] md:text-[48px] lg:text-[64px] font-medium text-white leading-[1.05] mb-6 tracking-tighter max-w-full lg:max-w-[80%] uppercase">
            {title}
          </h2>

          <p className="text-white/70 text-sm lg:text-lg font-medium leading-relaxed mb-10 line-clamp-2 max-w-[90%] md:max-w-[600px]">
            {description}
          </p>
